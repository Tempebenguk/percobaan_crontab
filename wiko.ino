...
IPAddress mikrotikRouterIp (10, 0, 10, 1);
String user = "admin";
String pwd = "admin";
String ssid = "WIFI COIN CLEON";
String password = "";
String adminAuth = "";
String vendorName = "WiFi Coin Cleon";
...
void disableCoinSlot(){
  ...
  digitalWrite(COIN_SET_PIN, LOW);
  digitalWrite(INSERT_COIN_LED, evaluateTriggerOutput(TURN_OFF));
  Serial.print("Coin Slot Disable");
      digitalWrite(ledPin, LOW);
...
int calculateAddTime(){
  ...
  while(remainingCoin > 0){
    ...
    if( candidateIndex != -1 ){
      ...
      //get the hi  lcd.init();
  lcd.backlight();ghest user rate profile from the rates
